...
### Dev Container in VSC

⚠️ Warning

A seemingly helpful prompt in the VS Code Dev Containers extension can trigger a problematic rebuild loop.

When selected, the extension repeatedly attempts to rebuild the Docker container, continuously creating cached images. While this initially appears harmless, it can consume all available disk space, leading to rebuild failures and, eventually, VS Code becoming unresponsive.

**Important**

When prompted by VS Code Dev Containers:

- Do not click ***Rebuild***
- Click ***Ignore*** instead

Correct Way to Rebuild Container

The only reliable and repeatable way to rebuild the container is via the project's `Makefile`.
...
If you have just cloned repo run:
...
> **NOTE:** Do Not
Do not use the VS Code Dev Containers extension to rebuild the Docker container. Doing so can result in broken development environment.

![Docker Rebuild](../images/visual_studio_docker_rebuild_ignore.png)  

![VSC Docker Rebuild Dialog](../images/vscode_dev_container_dialog.png)  
