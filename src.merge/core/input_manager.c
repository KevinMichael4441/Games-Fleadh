...
{
#ifdef PLATFORM_R36S
	TraceLog(LOG_INFO, "INPUT_MANAGER: Initialising R36S input manager");

	// Open R36S-specific input devices
	event_power_button = open("/dev/input/event0", O_RDONLY | O_NONBLOCK);
	if (event_power_button < 0)
		TraceLog(LOG_WARNING, "INPUT_MANAGER: Failed to open /dev/input/event0: %s", strerror(errno));
	else
		TraceLog(LOG_INFO, "INPUT_MANAGER: Opened /dev/input/event0 (Power button)");

	event_controller_button = open("/dev/input/event2", O_RDONLY | O_NONBLOCK);
	if (event_controller_button < 0)
		TraceLog(LOG_WARNING, "INPUT_MANAGER: Failed to open /dev/input/event2: %s", strerror(errno));
	else
		TraceLog(LOG_INFO, "INPUT_MANAGER: Opened /dev/input/event2 (Gamepad)");

	event_volume_buttons = open("/dev/input/event3", O_RDONLY | O_NONBLOCK);
	if (event_volume_buttons < 0)
		TraceLog(LOG_WARNING, "INPUT_MANAGER: Failed to open /dev/input/event3: %s", strerror(errno));
	else
		TraceLog(LOG_INFO, "INPUT_MANAGER: Opened /dev/input/event3 (Volume buttons)");
#else
	TraceLog(LOG_INFO, "INPUT_MANAGER: Initialising standard input manager");
#endif // PLATFORM_R36S
}
...
Command PollInput()
{
	...
	if (IsGamepadAvailable(0))
	{
		...
		if (gamepadActive)
		{
			...
			// Check thumbstick for directional input, prioritising vertical movement
			// Movement from Right stick
			if (fabsf(stick_Right_X) > TUMBSTICK_DEADZONE_THRESHOLD ||
				fabsf(stick_Right_Y) > TUMBSTICK_DEADZONE_THRESHOLD)
			{
				if (stick_Right_Y < -MOVE_VERTICAL_THRESHOLD)
					command |= AIM_UP;
				if (stick_Right_Y > MOVE_VERTICAL_THRESHOLD)
					command |= AIM_DOWN;
				if (stick_Right_X < -MOVE_HORIZONTAL_THRESHOLD)
					command |= AIM_LEFT;
				if (stick_Right_X > MOVE_HORIZONTAL_THRESHOLD)
					command |= AIM_RIGHT;
			}
...
// Menu and Start (see command.h)
	...
	if (command != NONE)
	{
		...
		TraceLog(LOG_INFO, "INPUT_MANAGER: Combined Command bits: %s", buffer);
	}
...
{
#ifdef PLATFORM_R36S
	TraceLog(LOG_INFO, "INPUT_MANAGER: Closing R36S input devices");
	if (event_power_button >= 0)
...
#else
	TraceLog(LOG_INFO, "INPUT_MANAGER: Cleaning up standard input manager");
#endif // PLATFORM_R36S
}