...
void InitTelemetry(Telemetry *r36s_telemetry)
{
	...
	r36s_telemetry->r36s_last_cpu_sample = -1e9;
	r36s_telemetry->r36s_last_memory_sample = -1e9;
	r36s_telemetry->r36s_last_temp_sample = -1e9;
	r36s_telemetry->r36s_last_battery_sample = -1e9;
...
void UpdateTelemetryFrame(Telemetry *r36s_telemetry, float deltaSec, int fps)
{
	r36s_telemetry->current_frame_ms = deltaSec * 1000.0f;
...
static void DrawBatteryBar(int x, int y, int w, int h, float percent)
{
    ...
    if (fill_width > (w - 2)) fill_width = (w - 2);
